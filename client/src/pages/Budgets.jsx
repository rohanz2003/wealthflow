...
export default function Budgets() {
  ...
  const handleDelete = async (id) => {
    ...
    try { await axios.delete(`/api/budgets/${id}`); fetchBudgets(); } catch (err) { console.error('Error:', err); }
  };
          ...
          <div key={b._id} className={`card p-5 card-hover ${b.isOverBudget ? 'border-red-300 dark:border-red-800 bg-red-50/30 dark:bg-red-900/10' : ''}`}>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2 ml-4">
                <button onClick={() => handleEdit(b)} className="p-2 text-gray-400 dark:text-navy-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg"><FiEdit2 size={16} /></button>
                <button onClick={() => handleDelete(b._id)} className="p-2 text-gray-400 dark:text-navy-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg"><FiTrash2 size={16} /></button>
              </div>
            </div>
          </div>
        ))}
